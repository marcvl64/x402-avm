...
vi.mock("algosdk", () => ({
  default: {
    ...
    makeAssetTransferTxnWithSuggestedParamsFromObject: vi.fn(),
    makeEmptyTransactionSigner: vi.fn(),
    assignGroupID: vi.fn(),
    encodeUnsignedTransaction: vi.fn(),
    decodeSignedTransaction: vi.fn(),
    decodeUnsignedTransaction: vi.fn(),
    encodeUnsignedSimulateTransaction: vi.fn(),
    AtomicTransactionComposer: vi.fn().mockImplementation(() => ({
      addTransaction: vi.fn(),
    })),
    modelsv2: {
      SimulateRequest: vi.fn(),
      SimulateRequestTransactionGroup: vi.fn(),
    },
    TransactionType: {
      pay: "pay",
      axfer: "axfer",
...
(algosdk.makePaymentTxnWithSuggestedParamsFromObject as any).mockReturnValue(mockTransaction);
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject as any).mockReturnValue(mockTransaction);
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.makeEmptyTransactionSigner as any).mockReturnValue({});
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.assignGroupID as any).mockReturnValue([mockTransaction]);
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.encodeUnsignedTransaction as any).mockReturnValue(new Uint8Array([1, 2, 3]));
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.encodeUnsignedSimulateTransaction as any).mockReturnValue(new Uint8Array([1, 2, 3]));

// Mock simulation classes
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.modelsv2.SimulateRequest as any).mockImplementation(() => ({}));
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(algosdk.modelsv2.SimulateRequestTransactionGroup as any).mockImplementation(() => ({}));
