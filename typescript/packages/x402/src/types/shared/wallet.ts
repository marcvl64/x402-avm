...
import { WalletAccount as AvmSigner } from "../../schemes/exact/avm/types";

export type ConnectedClient = evm.ConnectedClient | svm.SvmConnectedClient;
export type Signer = evm.EvmSigner | svm.SvmSigner | AvmSigner;
export type MultiNetworkSigner = { evm: evm.EvmSigner; svm: svm.SvmSigner; avm: AvmSigner };
...
/**
 * Checks if the given wallet is an AVM (Algorand) signer wallet
 *
 * @param wallet - The object wallet to check
 * @returns True if the wallet is an AVM signer wallet, false otherwise
 */
export function isAvmSignerWallet(wallet: Signer): wallet is AvmSigner {
  return (
