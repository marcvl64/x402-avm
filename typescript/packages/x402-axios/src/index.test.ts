...
import { evm, PaymentRequirements, ChainIdToNetwork, Signer, MultiNetworkSigner } from "x402-avm/types";
import { withPaymentInterceptor } from "./index";

// Mock the createPaymentHeader function
vi.mock("x402-avm/client", () => ({
...
describe("withPaymentInterceptor()", () => {
  ...
  beforeEach(async () => {
    ...
    const { selectPaymentRequirements } = await import("x402-avm/client");
  ...
  it("should handle 402 errors and retry with payment header", async () => {
    ...
    const { createPaymentHeader, selectPaymentRequirements } = await import("x402-avm/client");
  ...
  it("should reject if payment header creation fails", async () => {
    ...
    const { createPaymentHeader } = await import("x402-avm/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("x402-avm/types", async () => {
      const actual = await vi.importActual("x402-avm/types");
      return {
        ...actual,
